...
public class OrderDAO extends DataAccessObject<Order> {
    ...
    public Order findById(long id) {
        ...
        try(PreparedStatement statement = this.connection.prepareStatement(GET_BY_ID);){
            ...
            while(resultSet.next()){
                if(orderId==0){ // The orderID==0 indicates that order details have not been set. Default orderID is 0.
                    // if you just search for 1 orderId, you only need to set order detail once.
                    ...
                    order.setId(resultSet.getLong(4)); // get order id from resultSet
                    orderId = order.getId(); // update orderId for current order
    ...
    public List<Order> getOrdersForCustomer(long customerId){
        ...
        try(PreparedStatement statement = this.connection.prepareStatement(GET_FOR_CUST);){
            ...
            Order order = null;
            while(resultSet.next()){
                long localOrderId = resultSet.getLong(4); // get orderId from results, and set it as localOrderId
                if(orderId!=localOrderId){ // only set order details if orderId is not localOrderId, orderId can be seen
                    // as the orderId from last different order. or check if there is a new order.
                    order = new Order();
                    orders.add(order);
